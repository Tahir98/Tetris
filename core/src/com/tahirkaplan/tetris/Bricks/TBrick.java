...
public class TBrick extends Brick {
    ...
    public TBrick(int i, int j, int squareRowNum, int squareColumnNum, float size, Vector2 origin) {
        super(i, j, squareRowNum, squareColumnNum, size,origin);

        num = 4;

        texture = Main.manager.get("white.png",Texture.class);
    ...
    public void rotate(boolean[][] board) {
        if (rotation == Rotation.Def){
            ...
            if (i< squareColumnNum-1-1 && !board[i+1][j] && !board[i+2][j]){
            ...
            if (i>1 && !board[i-1][j] && !board[i-2][j+1]){
                rotation = Rotation.Def;
                setIndex(i-1,j);
            }
        }
