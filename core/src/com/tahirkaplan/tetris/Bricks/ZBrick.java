...
public class ZBrick extends Brick {
    ...
    private enum Rotation{
        Def,Up,Reversed,Down
    }

    private Rotation rotation;

    public ZBrick(int i, int j, int squareRowNum, int squareColumnNum, float size, Vector2 origin) {
        super(i, j, squareRowNum, squareColumnNum, size,origin);

        num = 4;

        texture = Main.manager.get("red.png",Texture.class);
    ...
    public void rotate(boolean[][] board) {
        if (rotation == Rotation.Def){
            ...
            if (i>= 2 && !board[i-1][j] && !board[i-2][j+1]){
        ...
        }else if (i <= squareColumnNum-1-2 && rotation == Rotation.Down){
